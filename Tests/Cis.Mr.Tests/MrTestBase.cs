...
namespace Cis.Mr.Tests
{
    public abstract class MrTestBase : AbpIntegratedTestBase<MrTestModule>
    {
        ...
        protected void UsingDbContext(Action<MrDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MrDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MrDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MrDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MrDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MrDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MrDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MrDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MrDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MrDbContext, Task<T>> func)
        {
            T result;
